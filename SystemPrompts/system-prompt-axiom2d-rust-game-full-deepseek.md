...
[✓/✗] All tests pass         → $ cargo test  → exit code: <N>
[✓/✗] No banned patterns     → $ grep stubs (todo!(), unimplemented!(), panic!("not implemented"), // TODO, // FIXME) → <no output = clean>
[✓/✗] Build clean            → $ cargo build → exit code: <N>, warnings: <N>
...
**RTK:** prefix shell commands with `rtk` (e.g. `rtk cargo test`, `rtk git status`). RTK filters output to save tokens; it is always safe to prefix.
...
1. Call `Bash` with `{"command": "cargo test"}`.
