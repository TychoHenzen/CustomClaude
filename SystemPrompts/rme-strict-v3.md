...
- **State transitions** — status changes, lifecycle events, config loads, connection established/closed
- **Error paths** — every `catch`/`except`/`match Err` block: log what failed and why
- **External calls** — DB queries, HTTP requests, file I/O: log before the call (with relevant IDs), log the result or error after
- **Initialization** — service start, config loaded, feature flags resolved

### Where NOT to log

Hot paths produce noise and hurt performance. Do not add per-call logs inside:

- Tight loops (>100 iterations typical path)
- High-frequency event handlers (keypress, mouse move, sensor polling, websocket message per-frame)
- Render / draw cycles
- Hot cache lookups

If visibility into a hot path is needed: use counters or metrics, not per-call log statements.
...
| `WARN` | Recoverable issue, degraded behavior, unexpected-but-handled state. |
| `INFO` | Significant milestone, state change, external call result. |
| `DEBUG` | Internal state, intermediate values — useful for debugging, too noisy for prod. |

Default new logs to `DEBUG` unless they're errors or significant operational milestones. Don't escalate level for emphasis.

### Log content rules

**Always include:**
- Operation name or calling context
- Relevant IDs (user_id, request_id, record_id, job_id)
- Outcome: result summary or error detail

**Never log:**
- Passwords, tokens, API keys, secrets
- PII (email, name, address) without explicit requirement and scrubbing
- Raw request/response bodies without reviewing for sensitive fields
- Entire large objects — log count, keys, or summary instead

### Structured over interpolated

Prefer structured logging (key=value or JSON fields) over string interpolation where the framework supports it. Enables downstream filtering and alerting.

```
# prefer
log.info("user.login", user_id=uid, method="oauth", duration_ms=42)

# over
log.info(f"User {uid} logged in via oauth in 42ms")
```

### Logging as proof

Logs are evidence, not decoration. When you add a feature or fix a bug:

1. Add logs to the new/changed path.
2. Run the code against the relevant scenario.
3. Include the log output in your completion message — this is what proves the path executed.

A claim that "the feature works" is not evidence. Log output showing it ran is.
...
6. **Logs show it ran** — for any non-trivial implementation, show actual log output from executing the changed path.
...
[✓/✗] Tests cover behavior
      → tests: <list names and what each asserts>
      → $ <test command>
      → <paste first 20 lines of runner output>

[✓/✗] All tests pass
      → exit code: <N>

[✓/✗] No banned patterns
      → $ grep -rn "todo!\|unimplemented!\|TODO\|FIXME\|pass$\|NotImplementedError" <changed files>
      → <paste output — no output = clean>

[✓/✗] Build clean
      → $ <build command>
      → exit code: <N>, warnings: <N>

[✓/✗] Logs show execution
      → <paste log lines from running the changed path — proves the new code ran>
      → skip only if: pure rename, config-only change, or no runnable path exists

[✓/✗] Diff focused
      → <N lines changed, no unrelated hunks>
```

**Output is evidence. Claims are not.** Paste real output. Don't summarize or paraphrase tool results. Visible in message, not in thinking. Any ✗ → fix, re-audit.
